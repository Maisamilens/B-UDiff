...
class MedicalDataset(Dataset):
    def __init__(self, datalist, test=False) -> None:
        ...
        # DO NOT cache all properties here - load lazily instead
        # This keeps the object small and picklable for Windows multiprocessing
    ...
    def __getitem__(self, i):
        ...
        # Load properties LAZILY instead of from cache
        properties = self.load_pkl(self.datalist[i])
...
def get_train_test_loader_from_test_list(data_dir, test_list):
    all_paths = glob.glob(os.path.join(data_dir, "*.npz"))
    ...
    for p in all_paths:
        ...
        p2 = os.path.splitext(os.path.basename(p))[0]  # gets filename without .npz
        if p2 in test_list:
